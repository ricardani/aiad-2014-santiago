...
public class Board implements Serializable {
    ...
    public boolean isIrrigated(int tileX, int tileY) {
        
        for (int j = 0; j < channel_array.length; ++j) {
            if (!channel_array[j].hasWater()) {
                break;
            }
            
            Vector<Pair<Integer, Integer>> vec = channel_array[j].getAdjacentTiles();
            for (int i = 0; i < vec.size(); ++i) {
                if (vec.get(i).getFirst().equals(tileX) && vec.get(i).getSecond().equals(tileY)) {
                    return true;
                }
            }
        }
